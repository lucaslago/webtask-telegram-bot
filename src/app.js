...
module.exports = (context, cb) => {
  ...
  if(/(^|\s)passado(\s|$)/.test(userText)) {
    sendMessage(token, chat, '"Temos que parar de escrever tanto, está difícil de ver o passado." - Autor Desconhecido');
  } else if(/(^|\s)(oi|ola|olá|hello|bom dia|Bom dia)(\s|$)/.test(userText)) {
    sendMessage(token, chat, 'Olá Jovem!');
  } else if(/(^|\s)(estilo|css|ensina css)(\s|$)/.test(userText)) {
    sendMessage(token, chat, 'Eu tenho estilos!');
  } else if(/(^|\s)(adeus|tchau|boa noite|flw)(\s|$)/.test(userText)) {
